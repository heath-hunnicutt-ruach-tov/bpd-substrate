...
PREFIX = os.environ.get("BPD_PREFIX", "imp")   # store unit id prefix
OUT = "/tmp/wg"
...
ORPHANS = set()
_PROD = os.path.join(BPD_ROOT, "producible3.txt")
if os.path.exists(_PROD):
    _age = time.time() - os.path.getmtime(_PROD)
    _ok = {l.strip() for l in open(_PROD) if l.strip()}
    ORPHANS = {os.path.basename(f)[5:-3] for f in files
               if os.path.basename(f)[5:-3] not in _ok}
...
_m = [t for f, t in zip(files, _m) if os.path.basename(f)[5:-3] not in ORPHANS]
...
for f in files:
    ...
    wp = os.path.abspath("wrapper_l3imp%s.py" % pid)
    try:
        r = subprocess.run([sys.executable, EMIT, kbf[0], PREFIX+pid, f],
                           capture_output=True, text=True, timeout=120)
        if r.returncode != 0 or not os.path.exists(wp):
            # the emitter may write elsewhere; look for it
            cand = glob.glob("wrapper_%s%s.py" % (PREFIX, pid))
            if cand: wp = cand[0]
            else:
