:- protocol(fusion_optimizerp).
    :- public(fuse_chain/2).
    :- public(estimate_savings/2).
    :- public(demonstrate/0).
:- end_protocol.

:- object(fusion_optimizer,
    implements(fusion_optimizerp)).

    :- uses(list, [append/3, select/3, member/2, length/2]).
