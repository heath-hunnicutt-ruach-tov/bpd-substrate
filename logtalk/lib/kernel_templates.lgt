%% File-level flag (was inside the module): double_quotes=codes is
%% SEMANTICALLY LOAD-BEARING — the template strings parse as code
%% lists. Logtalk objects reject set_prolog_flag as an entity
%% directive; placed at source-file level where it applies lexically.
:- set_prolog_flag(double_quotes, codes).

:- protocol(kernel_templatesp).
    :- public(generate_kernel_im2col/4).
    :- public(generate_kernel_reduction/4).
    :- public(generate_kernel_reduction/5).
    :- public(reduction_strategy_kind/1).
    :- public(reduction_strategy_description/2).
    :- public(generate_kernel_norm/4).
    :- public(generate_kernel_loss/4).
    :- public(generate_kernel_pool/5).
:- end_protocol.

:- object(kernel_templates,
    implements(kernel_templatesp)).
