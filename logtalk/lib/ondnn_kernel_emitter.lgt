:- protocol(ondnn_kernel_emitterp).
    :- public(emit_kernel/2).
:- end_protocol.

:- object(ondnn_kernel_emitter,
    implements(ondnn_kernel_emitterp)).

    :- uses(llvm_emit, [emit_declares/1, emit_globals/1, emit_strings/1, emit_type/2, emit_arglist/2, emit_declare/4, emit_header/2, emit_lazy_init/3, emit_install/2, llvm_extern/3]).
    :- uses(ondnn_jit_facts, [jit_insn/4, jit_const/3]).
