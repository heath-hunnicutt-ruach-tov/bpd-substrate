:- protocol(llvm_match_statusp).
    :- public(ulp_match/4).
    :- public(emission_pattern/3).
    :- public(reference_implementation/2).
    :- public(ulp_color/2).
    :- public(table_id/3).
    :- public(cell_dim_expression/4).
    :- public(emit_llvm_match_svg/1).
    :- public(main/0).
:- end_protocol.

:- object(llvm_match_status,
    implements(llvm_match_statusp)).

    :- uses(list, [length/2, nth0/3]).
