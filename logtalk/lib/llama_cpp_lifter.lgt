:- protocol(llama_cpp_lifterp).
    :- public(lift_arch_enum/2).
    :- public(lift_dispatch_table/2).
    :- public(lift_arch_full/3).
    :- public(lift_arch_tensors/2).
    :- public(lift_arch_graph_ops/2).
    :- public(lift_graph_aliases/2).
    :- public(arch_count/1).
    :- public(scan_template_params/4).
    :- public(parse_using_graph_line/3).
    :- public(parse_struct_declaration_line/2).
    :- public(match_prefix/3).
    :- public(find_substring_match/3).
    :- public(take_ident_chars/3).
    :- public(take_uc_ident_chars/3).
    :- public(skip_whitespace/2).
    :- public(is_ident_char/1).
    :- public(is_uc_ident_char/1).
    :- public(is_ident_boundary/1).
:- end_protocol.

:- object(llama_cpp_lifter,
    implements(llama_cpp_lifterp)).

    :- uses(c_ast, [emit_c/2, emit_program/2, emit_to_file/2, ast_uses_var/2]).


    :- uses(list, [append/2, append/3, member/2, length/2, nth0/3, reverse/2, subtract/3]).
...
lift_arch_enum(Path, Archs) :-
    {user:read_file_to_string(Path, Source, [])},
    split_string(Source, "\n", "", Lines),
...
lift_arch_tensors(SourcePath, Tensors) :-
    {user:read_file_to_string(SourcePath, Source, [])},
    re_foldl(
        [Dict, A0, A]>>(
...
parse_tensor_name(Atom, TensorRef) :-
    ...
    ( {user:re_matchsub("LLM_TENSOR_(?<name>[A-Z0-9_]+)},\\s*\"(?<part>[a-z]+)\",\\s*i", Str, Dict, [])}
    ...
    ; {user:re_matchsub("LLM_TENSOR_(?<name>[A-Z0-9_]+)},\\s*\"(?<part>[a-z]+)\"", Str, Dict, [])}
    -> get_dict(name, Dict, NameStr),
       get_dict(part, Dict, PartStr),
       atom_string(NameAtom, NameStr),
       downcase_atom(NameAtom, NameLc),
    ...
    ; {user:re_matchsub("LLM_TENSOR_(?<name>[A-Z0-9_]+)}", Str, Dict, [])}
    -> get_dict(name, Dict, NameStr),
       atom_string(NameAtom, NameStr),
       downcase_atom(NameAtom, NameLc),
...
lift_graph_aliases(ModelsHeaderPath, Aliases) :-
    {user:read_file_to_string(ModelsHeaderPath, Source, [])},
    split_string(Source, "\n", "", Lines),
