:- protocol(auto_fuserp).
    :- public(classify_op/2).
    :- public(fusion_plan/2).
    :- public(pipeline_depth/2).
    :- public(generate_fused_chain/2).
    :- public(demonstrate_chains/0).
:- end_protocol.

:- object(auto_fuser,
    implements(auto_fuserp)).

    :- uses(list, [member/2, length/2, max_list/2]).
