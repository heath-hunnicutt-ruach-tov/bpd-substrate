%% File-level: double_quotes=codes is PARSE-SEMANTICS for the 327
%% DCG rules' string literals (the kernel_templates precedent).
:- set_prolog_flag(double_quotes, codes).

:- protocol(c_astp).
    :- public(emit_c/2).
    :- public(emit_program/2).
    :- public(emit_to_file/2).
    :- public(ast_uses_var/2).
:- end_protocol.

:- object(c_ast,
    implements(c_astp)).

    %% THE EXTENSION POINT, now an explicit declared API (migration
    %% thesis): emit//2 was extended via Prolog multifile by
    %% c_preprocess_grammar_cpp (c_include/include_sys/header_guard
    %% grammar). Logtalk multifile-across-objects (design doc:
    %% HUB_SESSION_DESIGN.md — separate compilation units required).
    :- public(emit//2).
    :- multifile(emit//2).
    %% Grammar-extension support API: contributors' clause bodies call
    %% these (emit_atom for path/identifier rendering, emit_nodes for
    %% recursive emission). Public = the extension API is honest.
    :- public(emit_atom//1).
    :- public(emit_nodes//2).
