:- protocol(qkv_lifterp).
    :- public(lift_qkv_section/2).
    :- public(lift_ffn_section/2).
    :- public(lift_block_section/3).
    :- public(lift_ast_to_bpd/2).
    :- public(lift_ast_to_bpd_with_block/3).
    :- public(op_level_of/2).
:- end_protocol.

:- object(qkv_lifter,
    implements(qkv_lifterp)).

    :- uses(list, [append/2, append/3, reverse/2]).
