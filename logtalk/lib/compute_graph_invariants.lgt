:- protocol(compute_graph_invariantsp).
    :- public(assert_tensor/7).
    :- public(assert_op/4).
    :- public(check_all_invariants/1).
    :- public(check_view_containment/2).
    :- public(check_view_alignment/2).
    :- public(check_stride_consistency/2).
    :- public(check_write_read_coherence/3).
    :- public(check_temporal_ordering/3).
    :- public(check_shape_compatibility/3).
    :- public(diagnose_graph/1).
    :- public(clear_graph/0).
    :- public(check_dtype_coherence/3).
    :- public(check_dtype_flow/3).
    :- public(check_dtype_chain/2).
    :- public(trace_dtype_chain/2).
    :- public(check_scale_coherence/3).
    :- public(assert_scale_convention/2).
    :- public(scale_application_path/2).
    :- public(scale_matches_oracle/2).
:- end_protocol.

:- object(compute_graph_invariants,
    implements(compute_graph_invariantsp)).

    :- uses(list, [member/2, length/2, reverse/2, msort/2]).
