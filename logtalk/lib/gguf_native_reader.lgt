:- protocol(gguf_native_readerp).
    :- public(gguf_read/4).
    :- public(gguf_read/5).
    :- public(gguf_read_full/4).
    :- public(gguf_read_full/5).
    :- public(gguf_data_start/2).
    :- public(gguf_architecture_native/2).
    :- public(gguf_tensor_list/2).
:- end_protocol.

:- object(gguf_native_reader,
    implements(gguf_native_readerp)).
    ...
    %% safe_read is a migrated sibling object (batch 7).
    :- uses(safe_read, [safe_open/2, safe_close/1, safe_read_bytes/4,
        safe_read_uint8/3, safe_read_uint16_le/3, safe_read_uint32_le/3,
        safe_read_uint64_le/3, safe_read_int8/3, safe_read_int32_le/3,
        safe_read_float32_le/3, safe_read_string/3, safe_read_bool/3,
        safe_seek_and_read_bytes/5, safe_position/2, safe_verify_complete/1,
        safe_unclaimed_ranges/2, safe_claimed_bytes/2, safe_file_size/2]).
