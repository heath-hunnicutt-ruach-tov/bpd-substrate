%% soa_ffn_emitter.pl — Emit the full FFN layer as composed, verified atoms.
%%
%% RECONSTRUCTED 2026-08-31 (Iyun) from the June 2-6 corpus (conv 124 w375-377)
%% after the original was lost to /tmp-evaporation (never landed in git — the
%% revival-rule-one lesson incarnate).  Reconstruction, not recovery: confirmed
%% absent from every surviving tree before rebuilding.
%%
%% THE BY-CONSTRUCTION LADDER (RE-VECTOR rung 4).  The llama FFN block is:
%%
%%     norm      = ffn_norm(x)                       (provided upstream)
%%     gate      = W_gate · norm                     gemv   (atom)
%%     up        = W_up   · norm                     gemv   (atom)
%%     fused     = silu(gate) * up                   SwiGLU (SURVIVING atom)
%%     qfused    = quantize(fused)                   Q8_0   (atom)
%%     down      = W_down · qfused                   gemv   (atom)
%%
%% This emitter composes ATOMS, each independently verified 0-ULP:
%%   - @bpd_q8_0_dot        the proven block-dot   (SoA-structured)
%%   - @bpd_soa_gemv_q8_0   gemv = loop the dot over rows (rung 2, 0-ULP)
%%   - @bpd_swiglu_fused_cpu  SURVIVES in lib/swiglu_fused_emitter.pl
%%                            (divide form, scalar expf — the EXACT fusion the
%%                             June hand-kernel dropped; bit-identity by Iyun)
%%   - @bpd_q8_0_quantize   the rung-3 crux (d=amax/127, fp16(d), roundf(x·id)),
%%                          byte-exact re-quantization of the intermediate
%%
%% Because the FFN is a COMPOSITION of verified facts (not hand-steps), the
%% dropped-SwiGLU bug of June is STRUCTURALLY IMPOSSIBLE here: the fusion is a
%% linked atom, not a step that can be forgotten.  That is the whole point of
%% the by-construction track — the two-track insurance the June bug proved we need.
%%
%% The atoms are DECLARED as externals and linked at gate/build time (from the
%% surviving swiglu emitter + the landed/reconstructed gemv+quantize atoms).
%% This emitter's job is the COMPOSITION and its bit-identity-by-construction.
%%
%% VERIFY-BEFORE-CLAIMING: run this, llvm-as parse-check the emitted IR per
%% shape, then re-gate 0-ULP against the surviving tests/correctness framework.
%% Parsing is NOT passing — report the GATE results, not "it parses" (Bocher).

:- module(soa_ffn_emitter, [emit_soa_ffn/1, emit_soa_ffn/2]).
