...
# HYPOTHESIS UNDER TEST: the kernel quantises the activations by dividing by
# the F16 scale (dm), not the F32 scale (dq).  At near-ties that flips one
# element by one, and the observed delta is exactly that element's weight.
...
def model(a, use_dm):
    ...
    div = dm if use_dm else dq
    q = half_even(ab / np.where(div[:, None] > 0, div[:, None], 1)).astype(np.int32).clip(-127, 127)
...
print("   TEST: divide by the F16 scale (dm) instead of the F32 scale (dq)?")
print("   alternatives-differ count is the third column.")
tq = tm = td = 0
for tk in range(n):
    ...
    a_dq = model(one, False)
    a_dm = model(one, True)
    nq = int((a_dq == o).sum()); nm = int((a_dm == o).sum()); nd = int((a_dq != a_dm).sum())
    tq += nq; tm += nm; td += nd
    print("     tok %d: divide-by-dq %4d/%d   divide-by-dm %4d/%d   alts differ %4d"
          % (tk, nq, rows, nm, rows, nd))
print()
print("   TOTAL: divide-by-dq %d/%d   divide-by-dm %d/%d   alternatives differ on %d"
      % (tq, n * rows, tm, n * rows, td))
