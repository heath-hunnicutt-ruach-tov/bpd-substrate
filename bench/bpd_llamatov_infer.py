...
def build_model(gguf_path, n_layers=16):
    ...
    tensor_names = [
        "token_embd.weight",
        "output_norm.weight",
        "rope_freqs.weight",   # Llama 3 NTK-aware RoPE frequency factors (optional)
    ...
    # Load rope_freqs.weight (Llama 3 NTK-aware RoPE frequency factors).
    # Optional; if absent the freq_factors pointer is NULL.
    rope_freqs_arr = None
    if "rope_freqs.weight" in offsets:
        rope_freqs_arr = loader.load_f32(offsets["rope_freqs.weight"])
        print(f"[load] rope_freqs.weight loaded: shape=({rope_freqs_arr.shape[0]},), range=[{rope_freqs_arr.min():.2f}, {rope_freqs_arr.max():.2f}]")
